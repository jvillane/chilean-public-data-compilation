...
const publicFigureList: PublicFigureList = {};
for (const pf of publicFigures) {
  if (publicFigureList[pf.Id]) {
    console.log(`El id ${pf.Id} está repetido`);
  } else {
    const id = pf.Id;
    delete pf.Id;
    publicFigureList[id] = pf;
  }
}

writeFile("./data/public_figures.json", JSON.stringify(publicFigureList), function (err) {
  if (err) {
    console.log(err);
  }
});
